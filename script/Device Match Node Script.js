...
var devicePrintString = sharedState.get("clientDeviceProfile");
logger.message('devicePrintString : ' + devicePrintString);
...
var config = {
    ...
    attributes: {
        ...
        geolocation: {
            ...
            args: {
                allowedRange: 10,			//in miles
                penaltyPoints: 100
            }
        },
		clientDeviceIpAddress: {
			required: false,
			comparator: ScalarComparator,
			args: {
				penaltyPoints: 100
...
function compareDevicePrintProfiles(attributeConfig, devicePrint, devicePrintProfiles, maxPenaltyPoints) {
    ...
    results = [];
	var outerCounter=0, innerCounter=0;
    for (j = 0; j < devicePrintProfiles.length; j++) {
		logger.message("////////////////////////" + outerCounter++ + ".Device");
		...
		logger.message("////////////////////////" + innerCounter++ + ".Device Property");
    ...
    dao.saveDeviceProfiles(username, "/", vals);
...
function matchDevicePrint() {
    if (!username) {
        logger.message("Username not set. Cannot compare user's device print profiles.");
        outcome = "false";
    } else {
        ...
        var getProfiles = function () {
                ...
                function getNotExpiredProfiles() {
                    ...
                    profiles = sharedState.get('devicesJsonList');
                    logger.message('profiles : ' + profiles);
            ...
            devicePrint = JSON.parse(devicePrintString),
